...
func newStaticConfig() -> Apipb_OtsimoServices {
    ...
    let sc = try! Apipb_OtsimoServices(serializedData: data)
    return sc
}


extension Apipb_OtsimoServices {
    var clientConfig: ClientConfig {
        ...
        if let k = self.services["simplified-analytics"] {
            cc.simplifiedAnalyticsUrl = k
        }
        return cc
    }
}
...
open class ClusterConfig {
    ...
    fileprivate(set) open var config: Apipb_OtsimoServices?
    ...
    func storedData() -> Apipb_OtsimoServices? {
        let data = UserDefaults.standard.data(forKey: "OtsimoClusterConfig")
        if let d = data {
            do {
                return try Apipb_OtsimoServices(serializedData: d)
            } catch {
    ...
    func store(_ svc: Apipb_OtsimoServices) {
        config = svc
        do {
            let d = try svc.serializedData()
...
extension Otsimo {
    ...
    static func configFromDiscoveryService(_ serviceUrl: String, env: String, timeout: Double, handler: @escaping (_ cnf: ClientConfig?) -> Void) {
        ...
        var req = Apipb_DiscoveryRequest()
        req.osName = "ios"
        ...
        let RPC = discovery.get(req) { os, err in
            isCompleted = true
            if let e = err {
